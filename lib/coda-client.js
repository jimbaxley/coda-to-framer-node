...
function getCodaHeaders(apiToken, extraHeaders = {}) {
  return {
...
async function fetchCodaJson(url, apiToken) {
  ...
  const maxLatestAttempts = parseIntEnv(process.env.CODA_LATEST_VERSION_RETRY_ATTEMPTS || 20, 20, 1, 60);
  const latestDelayMs = parseIntEnv(process.env.CODA_LATEST_VERSION_RETRY_DELAY_MS || 1000, 1000, 100, 10000);
  let lastError;

  for (let attempt = 1; attempt <= Math.max(maxAttempts, maxLatestAttempts); attempt += 1) {
    try {
      const response = await fetch(url, {
        headers: getCodaHeaders(apiToken),
      });
      ...
      const allowedAttempts = pendingMutation ? maxLatestAttempts : maxAttempts;
      if (!retryable || attempt >= allowedAttempts) {
        break;
      }

      const waitMs = computeBackoffMs(pendingMutation ? latestDelayMs : baseDelayMs, attempt);
      const message = error instanceof Error ? error.message : String(error);
      log("warn", pendingMutation ? "latest_version_retry" : "transient_api_retry", {
        attempt,
...
export async function updateTableCell(docId, tableIdOrName, rowId, columnId, value, apiToken) {
  ...
  // ceiling.
  const maxAttempts = parseIntEnv(process.env.CODA_CALLBACK_UPDATE_RETRY_ATTEMPTS || 5, 5, 1, 10);

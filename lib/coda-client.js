...
function getCodaMetadataOptions(options = {}) {
  return options.latestRowsOnly ? { ...options, requireLatest: false } : options;
}

function getCodaRowsOptions(options = {}) {
  return { ...options, requireLatest: Boolean(options.requireLatest) };
}
...
export async function resolveColumnNameOrId(docId, tableIdOrName, columnNameOrId, apiToken, options = {}) {
  // Already a column ID — no network call needed.
  if (/^c-[A-Za-z0-9_-]+$/.test(String(columnNameOrId || ""))) {
    return columnNameOrId;
  }

  // Try the direct single-column endpoint first — avoids fetching all columns for a large table.
  // Coda accepts either a column ID or name in this path segment.
  try {
    const url = `${CODA_API_BASE}/docs/${encodeURIComponent(docId)}/tables/${encodeURIComponent(tableIdOrName)}/columns/${encodeURIComponent(columnNameOrId)}`;
    const col = await fetchCodaJson(url, apiToken, options);
    if (col?.id) {
      log("info", "resolve_column_by_direct_fetch", { docId, tableIdOrName, columnNameOrId, resolvedColumnId: col.id });
      return col.id;
    }
  } catch (_) {
    // fall through to full column list
  }
  ...
  if (byId) {
    log("info", "resolve_column_by_id", { docId, tableIdOrName, columnNameOrId, resolvedColumnId: byId.id });
    return byId.id;
  }

  const byName = columns.find(c => c.name === columnNameOrId);
  if (byName) {
    log("info", "resolve_column_by_name", { docId, tableIdOrName, columnNameOrId, resolvedColumnId: byName.id });
    return byName.id;
  }
...
export async function getCodaTableDataPaginated(docId, tableIdOrName, apiToken, options = {}, maxRows = 10000) {
  ...
  const columnsStartedAt = Date.now();
  const columnsPromise = getCodaTableColumns(docId, tableIdOrName, apiToken, options)
    .then((columns) => {
      log("info", "table_columns_read", {
        docId,
        tableIdOrName,
        columnsCount: columns.length,
        elapsedMs: Date.now() - columnsStartedAt,
      });
      return columns;
    });
  const rows = [];
  ...
  log("info", "table_rows_read", {
    docId,
    tableIdOrName,
    rowsCount: rows.length,
    pages: page,
    elapsedMs: Date.now() - rowsStartedAt,
  });
  log("info", "table_snapshot_read", {
    docId,
    tableIdOrName,
    columnsCount: columns.length,
    rowsCount: rows.length,
...
export async function getCodaTableData(docId, tableIdOrName, apiToken, limit = 100, options = {}) {
  ...
  const columnsStartedAt = Date.now();
  const rowsStartedAt = Date.now();
  const [columns, rowsData] = await Promise.all([
    getCodaTableColumns(docId, tableIdOrName, apiToken, options).then((columns) => {
      log("info", "table_columns_read", {
        docId,
        tableIdOrName,
        columnsCount: columns.length,
    ...
    fetchCodaJson(rowsUrl, apiToken, options).then((rowsData) => {
      log("info", "table_rows_read", {
        docId,
        tableIdOrName,
  ...
  log("info", "table_snapshot_read", {
    docId,
    tableIdOrName,
    columnsCount: snapshot.columns.length,
...
// Fetch rows matching a column value using Coda's server-side query filter.
// Much faster than fetching all rows when looking up a single row by slug.
export async function getCodaRowByColumnValue(docId, tableIdOrName, columnId, value, apiToken, options = {}) {
  const query = `${encodeURIComponent(columnId)}:"${encodeURIComponent(String(value))}"`;
  const rowsUrl = `${CODA_API_BASE}/docs/${encodeURIComponent(docId)}/tables/${encodeURIComponent(tableIdOrName)}/rows?useColumnNames=true&valueFormat=rich&limit=10&query=${query}`;
  const startedAt = Date.now();
  const columnsStartedAt = Date.now();
  const rowsStartedAt = Date.now();
  const metadataOptions = getCodaMetadataOptions(options);
  const rowsOptions = getCodaRowsOptions(options);

  const [columns, rowsData] = await Promise.all([
    getCodaTableColumns(docId, tableIdOrName, apiToken, metadataOptions).then((columns) => {
      log("info", "table_columns_read", {
        docId,
        tableIdOrName,
        columnsCount: columns.length,
        requireLatest: Boolean(metadataOptions.requireLatest),
        elapsedMs: Date.now() - columnsStartedAt,
      });
      return columns;
    }),
    fetchCodaJson(rowsUrl, apiToken, rowsOptions).then((rowsData) => {
      log("info", "table_rows_query_read", {
        docId,
        tableIdOrName,
        columnId,
        rowsCount: (rowsData.items || []).length,
        requireLatest: Boolean(rowsOptions.requireLatest),
        elapsedMs: Date.now() - rowsStartedAt,
      });
      return rowsData;
    }),
...
export async function getCodaRowData(docId, tableIdOrName, rowId, apiToken, options = {}) {
  ...
  const metadataOptions = getCodaMetadataOptions(options);
  const rowsOptions = getCodaRowsOptions(options);
  const [columns, rowData] = await Promise.all([
    getCodaTableColumns(docId, tableIdOrName, apiToken, metadataOptions).then((columns) => {
      log("info", "table_columns_read", {
        docId,
        tableIdOrName,
    ...
    fetchCodaJson(rowUrl, apiToken, rowsOptions).then((rowData) => {
      log("info", "table_row_read", {
        docId,
        tableIdOrName,
        rowId,
        requireLatest: Boolean(rowsOptions.requireLatest),
...
export async function updateTableCell(docId, tableIdOrName, rowId, columnId, value, apiToken, options = {}) {
  ...
  for (let attempt = 1; attempt <= Math.max(retrySettings.maxAttempts, retrySettings.maxLatestAttempts); attempt += 1) {
    try {
      ...
      if (cellResponse.ok) {
        ...
        if (options.waitForMutation !== false) {

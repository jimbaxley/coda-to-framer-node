...
export function buildFieldsAndItems({
  ...
  // Exclude lookup fields from setFields — Framer can't create cross-collection reference fields via managed API
  const fields = allMappedFields.filter((field) => field.type !== "lookup");

  // But include lookup fields when building item fieldData

...
function markdownToSanitizedHtml(markdown) {
  // DEBUG: Log the raw markdown input to diagnose special characters
  const debugInfo = {
    markdown,
    length: markdown.length,
    charCodes: Array.from(markdown).map((char, i) => ({ index: i, char, code: char.charCodeAt(0) })).filter((item) => item.code > 127 || item.char === '`'),
  };
  console.log("[markdownToSanitizedHtml] DEBUG:", JSON.stringify(debugInfo, null, 2));

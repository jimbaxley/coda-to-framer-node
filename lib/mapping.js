...
function extractCodaReferenceIds(value) {
  const ids = [];
  ...
  const visit = (candidate) => {
    ...
    if (candidate.type === "ref" && typeof candidate.identifier === "string") {
      ids.push(candidate.identifier);
      return;
    }

    if (candidate.additionalType === "row" && typeof candidate.rowId === "string") {
      ids.push(candidate.rowId);
      return;
    }

    if ("rawValue" in candidate) visit(candidate.rawValue);
    if ("value" in candidate) visit(candidate.value);
  };
...
function isCodaMultiReferenceValue(value) {
  if (Array.isArray(value)) return true;
  if (!value || typeof value !== "object") return false;
  if ("rawValue" in value && isCodaMultiReferenceValue(value.rawValue)) return true;
  if ("value" in value && isCodaMultiReferenceValue(value.value)) return true;
  return false;
}
...
// Returns only the resolved reference fields (lookup type → multiCollectionReference/collectionReference).
// Non-lookup fields are excluded — they're already in mappingResult.fields.
export function resolveReferenceFields(
  allMappedFields,
  codaTableIdToFramerCollectionId,
  codaFieldIdToFramerCollectionId = new Map(),
) {
  return allMappedFields
    .filter((field) => field.type === "lookup")
    .map((field) => {
      const framerCollectionId =
        codaFieldIdToFramerCollectionId.get(field.id) ||
        (field.codaRefTableId
          ? codaTableIdToFramerCollectionId.get(field.codaRefTableId)
          : null);
      if (!framerCollectionId) return null;
      return {
        id: field.id,
        name: field.name,
...
function transformCodaValue(value, field, codaColumnType, use12HourTime) {
  ...
  switch (field.type) {
    ...
    case "lookup": {
      ...
      const ids = extractCodaReferenceIds(value);
...
export function buildFieldsAndItems({
  ...
  const allMappedFields = columns
    ...
    .map((field) => {
      if (field.type !== "lookup" || field.multiple) return field;
      const isMultiple = rows.some((row) =>
        isCodaMultiReferenceValue(row.values?.[field.name]),
      );
      return isMultiple ? { ...field, multiple: true } : field;
    });

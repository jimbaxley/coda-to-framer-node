...
export function buildFieldsAndItems({
  ...
  rows.forEach((row, index) => {
    ...
    const slugValue = extractSlugValue(slugFieldValue);
    if (!slugValue) {
      skippedCount += 1;
      const warning = `Row ${rowId} slug extraction failed. slugColumnName=${slugColumnName}, slugFieldValue=${JSON.stringify(slugFieldValue)}`;

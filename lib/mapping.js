...
export function mapCodaTypeToFramerType(column) {
  ...
  const isLinkListColumn = compactName === "imagelinks" || compactName === "textlinks";

  if (baseType === "button") {
    return null;
  }

  if ((baseType === "url" || baseType === "link") && isLinkListColumn) {
    return {
      id: column.id,
      name: column.name,
      type: "link",
  ...
  if (
    baseType === "image" ||
    (!isLinkListColumn && name.includes("image")) ||
...
function extractReferenceIds(value) {
  ...
  const processItem = (item) => {
    ...
    if (item && typeof item === "object") {
      if (typeof item.rowId === "string") return item.rowId;
...
function transformCodaValue(value, field, codaColumnType, use12HourTime) {
  ...
  switch (field.type) {
    ...
    case "multiCollectionReference":
      return extractReferenceIds(value);
    default:
      return value;
  }
}
...
export function buildFieldsAndItems({
  ...
  const fields = columns
    .map((column) => {
      ...
      const baseType = String(column.format?.type || "").toLowerCase();
      const referencedTableId = column.format?.table?.id;
      const linkedCollectionId = referencedTableId ? referenceMap.get(referencedTableId) : undefined;

      if (
        mappedField &&
        mappedField.type === "string" &&
        (baseType === "lookup" || baseType === "reference") &&
        linkedCollectionId
      ) {
        mappedField = {
          id: column.id,

  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const result = await withFramer(projectUrl, apiKey, async (framer) => {
        if (attempt > 1) {
          log("info", "publish_retry", { projectUrl, attempt, maxAttempts });
        } else {
          log("info", "publish_connected", { projectUrl });
        }

        log("info", "publish_running", { projectUrl, attempt });
        let publishResult;
        try {
          ...
          log("error", "publish_failed", { projectUrl, attempt, ...formatted.fields });
        ...
        log("info", "deploy_running", { projectUrl, deploymentId });
        try {
          await withTimeout(framer.deploy(deploymentId), timeoutMs, "deployProject");
        } catch (error) {
          const formatted = formatFramerError("deploy", error);
          log("error", "deploy_failed", { projectUrl, deploymentId, attempt, ...formatted.fields });
          throw new Error(formatted.message);
        }
        log("info", "deploy_complete", { projectUrl, deploymentId });
      ...
      if (attempt < maxAttempts) {
        log("warn", "publish_transient_retry", { projectUrl, attempt, maxAttempts, retryDelayMs, message });

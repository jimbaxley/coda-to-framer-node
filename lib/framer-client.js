...
async function withFramer(projectUrl, apiKey, fn) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      console.warn(`[framer] Transient connection issue (attempt ${attempt}/${maxAttempts}): ${message}. Retrying in ${waitMs}ms...`);
      await sleep(waitMs);
    }
  }
...
export async function addItemsToCollection(
  ...
  const addBatchWithFieldRetry = async (
    ...
    try {
      ...
      if (missingFieldKey && !removedFieldKeys.has(missingFieldKey)) {
        ...
        console.warn(
          `[framer] Field ${missingFieldKey} not present in collection. Retrying ${operationName} without it...`,
        );
  ...
  return withFramer(projectUrl, apiKey, async (framer) => {
    ...
    try {
      ...
      console.warn(`[framer] Bulk addItems failed (${message}). Falling back to ${fallbackMode} add...`);
    }
